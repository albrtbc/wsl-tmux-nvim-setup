...
class BackupManager:
    ...
    def create_backup(self, source_paths: List[Path], backup_name: str,
                     description: str = "", exclude_patterns: Optional[List[str]] = None) -> Path:
        """
        ...
        try:
            ...
            try:
                ...
                if self.show_progress:
                    ...
                    self.console.print(f"[green]✓ Backup created: {backup_filename} ({size_str})[/green]")
    ...
    def restore_backup(self, backup_name: str, restore_to: Optional[Path] = None,
                      backup_timestamp: Optional[str] = None) -> Path:
        """
        Restore from backup
    ...
    def delete_backup(self, backup_name: str, backup_timestamp: Optional[str] = None,
        ...
        if confirm and self.show_progress:
            ...
            self.console.print(f"[yellow]About to delete {count} backup(s) for '{backup_name}'[/yellow]")
            ...
            if backup_timestamp:
                ...
                if backup_name in metadata:
                    metadata[backup_name]['backups'] = [
                        b for b in metadata[backup_name]['backups']
    ...
    def get_backup_info(self, backup_name: str, backup_timestamp: Optional[str] = None) -> Dict[str, Any]:
        ...
        if backup_path.exists():
            ...
            try:
                ...
                info.update({
                    ...
                    'uncompressed_size_human': self._format_size(archive_info['uncompressed_size'])
                })
            except Exception:
    ...
    def cleanup_orphaned_backups(self) -> int:
        ...
        for orphaned_file in orphaned:
            try:
                ...
                deleted_count += 1
                if self.show_progress:
                    self.console.print(f"[yellow]Deleted orphaned backup: {orphaned_file.name}[/yellow]")
            except Exception as e:
                if self.show_progress:
    ...
    def _copy_to_staging(self, source_paths: List[Path], staging_dir: Path,
        ...
        try:
            for i, source_path in enumerate(source_paths):
                ...
                if not source_path.exists():
                    if self.show_progress:
                        self.console.print(f"[yellow]Warning: {source_path} does not exist[/yellow]")
                    continue
    ...
    def _copy_tree_with_excludes(self, source_dir: Path, dest_dir: Path,
    ...
    def _record_backup_metadata(self, backup_name: str, backup_path: Path,
                               source_paths: List[Path], description: str) -> None:
        """Record backup metadata"""
        metadata = self._load_backup_metadata()
    ...
    def _find_backup(self, backup_name: str, timestamp: Optional[str] = None) -> Optional[Dict[str, Any]]:
