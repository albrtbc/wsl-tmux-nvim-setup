...
class NetworkResilientDownloader:
    ...
    def check_mirror_health(self, mirror: Mirror) -> bool:
        ...
        mirror.success_rate = mirror.successful_requests / mirror.total_requests if mirror.total_requests > 0 else 0.0
        return False
        ...
        for attempt in range(task.max_retries):
            try:
                ...
                if attempt < task.max_retries - 1:
                    ...
                    wait_time = 2 ** attempt
                    self.console.print(f"[yellow]Network error, retrying in {wait_time}s: {e}[/yellow]")
    ...
    def download_file(self, task: DownloadTask, show_progress: bool = True) -> bool:
        ...
        if resume_pos > 0:
            ...
            try:
                ...
                if response.status_code not in [206, 416]:  # Partial content or range not satisfiable
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats['healthy_mirrors'] = len([m for m in self.mirrors.values() if m.status == MirrorStatus.HEALTHY])
...
def create_network_downloader(cache_dir: Optional[Path] = None, 
                            offline_mode: bool = False,
                            console: Optional[Console] = None) -> NetworkResilientDownloader:
