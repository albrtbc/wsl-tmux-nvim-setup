...
try:
    from upload_release import GitHubReleaseClient, GitHubAPIError, ReleaseAsset
except ImportError as e:
...
class GitHubClient:
    ...
    def __init__(self, token: Optional[str] = None, repository: str = "albertodall/wsl-tmux-nvim-setup"):
    ...
    def list_releases(self, include_prerelease: bool = False, 
                     include_draft: bool = False, limit: int = 30) -> List[Dict[str, Any]]:
        """
    ...
    def download_release_asset(self, download_url: str, output_path: Path,
    ...
    def search_releases(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for release in releases:
            if (query_lower in release['tag_name'].lower() or 
                query_lower in release['name'].lower()):
    ...
    def format_release_info(self, release: Dict[str, Any], detailed: bool = False) -> str:
        ...
        lines = []
        lines.append(f"[bold cyan]{escape(release['name'])}[/bold cyan] ([dim]{release['tag_name']}[/dim])")
...
if __name__ == '__main__':
    ...
    @click.option('--repository', default='albertodall/wsl-tmux-nvim-setup', 
