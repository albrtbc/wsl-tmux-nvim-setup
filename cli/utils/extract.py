...
class ExtractManager:
            ...
            try:
                ...
                for member_name in members:
                    ...
                    if not self._is_safe_path(extract_to, full_path):
                        if self.show_progress:
                            self.console.print(
                                f"[yellow]Skipping unsafe path: {member_name}[/yellow]"
                            )
                        continue
    ...
    def _create_zip_archive(
        self, source_dir: Path, archive_path: Path, exclude_patterns: List[str]
    ) -> Path:
        """Create ZIP archive"""
    ...
    def extract(archive_path, extract_to, strip_components):
        ...
        try:
            ...
            result = manager.extract_archive(
                Path(archive_path), Path(extract_to), strip_components=strip_components
            )
