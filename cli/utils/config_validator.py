...
class EnhancedConfigModel(BaseModel):
    ...
    performance_profile: str = Field(
        default="auto", regex="^(auto|low_resource|standard|high_performance)$"
    )
    max_workers: int = Field(default=0, ge=0, le=32)  # 0 = auto-detect

    # Component preferences
    preferred_components: List[str] = Field(
        default_factory=lambda: ["dependencies", "tmux", "neovim", "yazi", "lazygit"]
    )
...
class ConfigValidator:
            ...
            if section not in config:
                self.issues.append(
                    ConfigIssue(
                        ...
                        suggested_value=(
                            "~/.local/share/wsl-tmux-nvim-setup"
                            if section == "installation_path"
                            else None
                        ),
                        auto_fixable=True,
                    )
    ...
    def _validate_network_settings(self, config: Dict[str, Any]):
        ...
        if github_token and self.validation_level in [
...
class ConfigMigrator:
    ...
    def migrate_config(
        self, old_config: Dict[str, Any], target_version: str = "latest"
    ) -> Dict[str, Any]:
        ...
        self.console.print(
            f"[blue]Migrating configuration from version {current_version} to {target_version}[/blue]"
        )
