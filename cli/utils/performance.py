...
class PerformanceCache:
    ...
    def _evict_lru(self):
        ...
        with self.lock:
            while (self.stats.memory_usage > self.max_size or 
...
class ResourceMonitor:
    ...
    def get_resource_trend(self, minutes: int = 5) -> Dict[str, float]:
        ...
        recent_samples = [r for r in self.resource_history 
                         if datetime.now() - timedelta(seconds=len(self.resource_history) - 
                         self.resource_history.index(r)) >= cutoff_time]
...
class PerformanceOptimizer:
    ...
    def __init__(self, console: Optional[Console] = None):
        ...
        self.cache = PerformanceCache(
            max_size=self.profile.cache_size * 1024 * 1024,
            max_entries=1000
        )
