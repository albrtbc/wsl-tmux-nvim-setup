...
from utils.github import GitHubClient, GitHubAPIError
from utils.download import DownloadManager, DownloadError
from utils.extract import ExtractManager, ExtractionError
from utils.backup import BackupManager, BackupError
from utils.version_utils import VersionComparator, ComponentVersionManager
...
class InstallManager:
    ...
    def download_release_assets(self, release_info: Dict[str, Any], 
                               download_dir: Path) -> Dict[str, Path]:
    ...
    def extract_and_install(self, archive_path: Path, version_info: Dict[str, Any]) -> bool:
        ...
        with tempfile.TemporaryDirectory(prefix="wsm_install_") as temp_dir:
            ...
            try:
                ...
                self.console.print(f"[blue]Running installation script: {install_script.name}[/blue]")
                ...
                if result.returncode != 0:
                    ...
                    raise click.ClickException(f"Installation script failed (exit code: {result.returncode})")
    ...
    def update_installation_status(self, version_info: Dict[str, Any], 
        ...
        if components is None:
            ...
            components = ['tmux', 'neovim', 'yazi', 'lazygit', 'synth-shell', 'nerdfonts', 'kitty']
    ...
    def install_version(self, version: Optional[str] = None, 
...
@click.option('--force', is_flag=True, 
              help='Force installation even if same version is installed')
...
@click.option('--interactive', is_flag=True,
              help='Run interactive installation wizard')
@click.option('--ui-mode', type=click.Choice(['rich', 'curses', 'textual', 'auto'], case_sensitive=False),
              default='auto', help='UI mode for interactive installation')
@click.pass_obj
def install(ctx_obj, version, prerelease, force, skip_backup, components, list_versions, interactive, ui_mode):
    """
    ...
    try:
        ...
        if interactive or (not version and not components and not list_versions):
            ...
            from interactive.textual_installer import create_textual_installer, textual_available
