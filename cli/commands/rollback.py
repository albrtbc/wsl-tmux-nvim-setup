...
class RollbackManager:
    ...
    def rollback_from_backup(self, backup_name: str, 
        ...
        try:
            ...
            try:
                ...
                if current_backup:
                    self.console.print(f"[green]✓ Current state backed up[/green]")
    ...
    def rollback_to_version(self, version: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if not force:
                self.console.print(f"[yellow]Warning: Could not compare versions. Use --force to proceed.[/yellow]")
                return False
        ...
        rollback_panel = Panel(
            ...
            f"Operation:       [yellow]Version Rollback[/yellow]",
...
@click.option('--backup', '-b', type=str,
              help='Rollback from specific backup name')
@click.option('--version', '-v', type=str,
              help='Rollback to specific version')
@click.option('--force', is_flag=True,
              help='Force rollback without version checks')
@click.option('--restore-to', type=click.Path(),
              help='Custom restore location for backup rollback')
@click.pass_obj
    ...
    try:
        ...
        if list or (not target and not backup and not version):
            ...
            if not list and options:
                ...
                console.print(f"   wsm rollback --backup <backup_name>")
                console.print(f"   wsm rollback --version <version>")
                console.print(f"   wsm rollback <version>[/dim]")
