...
class ListManager:
    ...
    def list_releases(self, include_prerelease: bool = False, 
                     include_draft: bool = False, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    def show_releases_table(self, releases: List[Dict[str, Any]], 
...
@click.option('--draft', is_flag=True,
              help='Include draft releases (if you have access)')
...
@click.option('--search', '-s', type=str,
              help='Search releases by query string')
@click.option('--version', '-v', type=str,
              help='Show details for specific version')
@click.option('--compare', is_flag=True,
              help='Show version comparison with current installation')
@click.pass_obj
def list_releases(ctx_obj, prerelease, draft, limit, detailed, search, version, compare):
    ...
    try:
        ...
        console.print(f"💡 Use 'wsm install <version>' to install a specific version[/dim]")
