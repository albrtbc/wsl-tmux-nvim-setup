    ...
    def show_installation_status(self, detailed: bool = False) -> None:
        ...
        if status['version'] != 'unknown':
            ...
            if status['installation_date']:
                ...
                try:
                    install_date = datetime.fromisoformat(status['installation_date'].replace('Z', '+00:00'))
            ...
            if status['last_update']:
                try:
                    update_date = datetime.fromisoformat(status['last_update'].replace('Z', '+00:00'))
            ...
            status_lines = [
                ...
                "Run 'wsm install' to get started"
            ]
    ...
    def show_configuration_status(self) -> None:
        ...
        for name, info in config_checks.items():
            if name == 'wsm_config':
                ...
                status_display = "[green]✓ Valid[/green]" if info.get('valid') else "[red]✗ Invalid[/red]"
                size_display = ""
        ...
        settings_table.add_row("Auto Update:", "✓ Enabled" if self.config.auto_update else "✗ Disabled")
        settings_table.add_row("Backup Retention:", f"{self.config.backup_retention} backups")
        settings_table.add_row("GitHub Token:", "✓ Set" if self.config.github_token else "✗ Not set")
...
@click.option('--updates', '-u', is_flag=True,
              help='Check update status')
@click.option('--all', '-a', is_flag=True,
              help='Show all information (equivalent to -d -c --config -u)')
@click.pass_obj
def status(ctx_obj, detailed, components, config, updates, all):
    ...
    try:
        ...
        if not any([detailed, components, config, updates]):
            ...
            console.print(f"💡 Use --all to show complete status information[/dim]")
