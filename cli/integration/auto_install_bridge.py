...
class AutoInstallBridge:
    ...
    def is_component_installed(self, component_name: str) -> bool:
        ...
        try:
            ...
            if status.installed_components and component_name.lower() in [
    ...
    def install_components(
        ...
        self.console.print(
            f"[blue]Installing {len(component_names)} components using {mode.value} mode[/blue]"
        )
        ...
        if show_progress:
            with Progress(
                ...
                main_task = progress.add_task(
                    "Installing components...", total=len(valid_components)
                )
                ...
                for i, component_name in enumerate(valid_components):
                    ...
                    if success:
                        ...
                        progress.console.print(
                            f"[green]✓ {component.name} installed successfully[/green]"
                        )
                    else:
    ...
    def _display_installation_summary(self):
        ...
        if failed == 0:
            self.console.print(
                f"[green]🎉 All {successful} components installed successfully![/green]"
            )
        else:
            self.console.print(
                f"[yellow]⚠ {successful}/{total} components succeeded, {failed} failed[/yellow]"
            )
...
def create_auto_install_bridge(
