...
class TestInputValidation:
    ...
    def test_version_string_validation(self):
        ...
        for version in invalid_versions:
            assert not version_pattern.match(version), f"Invalid version {version} passed validation"
    ...
    def test_github_token_validation(self):
        ...
        for token in valid_tokens:
            assert validate_github_token(token), f"Valid token format failed validation"
        
        # Test invalid tokens
        for token in invalid_tokens:
            assert not validate_github_token(token), f"Invalid token format passed validation"
...
class TestPathTraversalPrevention:
    ...
    def test_extract_path_validation(self, temp_dir):
        ...
        # Test safe paths
        for path in safe_paths:
            assert is_safe_extract_path(path, temp_dir), f"Safe extraction path {path} failed validation"
        
        # Test dangerous paths
        for path in dangerous_paths:
            assert not is_safe_extract_path(path, temp_dir), f"Dangerous extraction path {path} passed validation"
    ...
    def test_config_file_path_validation(self, temp_dir):
        ...
        for path in safe_config_paths:
            assert is_safe_config_path(path, config_dir), f"Safe config path {path} failed validation"
        
        # Test dangerous configuration paths  
        for path in dangerous_config_paths:
            assert not is_safe_config_path(path, config_dir), f"Dangerous config path {path} passed validation"
...
class TestSecureFileOperations:
    ...
    def test_temporary_file_security(self, temp_dir):
        ...
        try:
            ...
            assert file_permissions & 0o044 == 0, "Temporary file should not be readable by group or others"
    ...
    def test_backup_file_security(self, temp_dir):
        ...
        assert backup_permissions == original_permissions, "Backup should have same permissions as original"
