...
class TestCIWorkflowJobs:
    ...
    def test_security_scan_job(self, ci_workflow):
        ...
        assert any('safety' in name.lower() or 'vulnerabilit' in name.lower() for name in step_names)
    ...
    def test_documentation_job(self, ci_workflow):
        ...
        assert any('markdown' in name.lower() or 'documentation' in name.lower() for name in step_names)
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    for script in shell_scripts[:3]:  # Test first 3 scripts
                        ...
                        assert result.returncode in [0, 1], f"ShellCheck failed on {script}"
            ...
            for pattern in secret_patterns:
                ...
                for match in matches:
                    ...
                    if '${{' not in match.group():
                        pytest.fail(f"Potential hardcoded secret in {workflow_file.name}: {match.group()}")
                ...
                for step in job['steps']:
                    if 'uses' in step:
                        ...
                        if 'actions/checkout' in action:
                            ...
                            assert '@v' in action or '@main' not in action, \
            ...
            if script_path.exists():
                ...
                if script_path.suffix == '.py':
                    ...
                    try:
                        ...
                        assert result.returncode == 0, f"Shell script {script_path} has syntax errors"
                    except Exception as e:
                        pytest.skip(f"Could not test {script_path}: {e}")
