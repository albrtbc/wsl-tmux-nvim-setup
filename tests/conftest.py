...
def mock_version_manager():
    ...
    with patch('cli.utils.version_utils.ComponentVersionManager') as mock_class:
        ...
        mock_manager.version_manager.get_latest_release = Mock(return_value={
            'tag_name': 'v1.1.0',
            'name': 'Release v1.1.0',
        ...
        # Component versions
        mock_manager.get_component_versions = Mock(return_value={
            'tmux': '3.3a',
            'neovim': '0.9.0',
            'git': '2.40.0'
        })
        ...
        mock_manager.check_system_compatibility = Mock(return_value={
            'wsl_available': True,
        ...
        mock_manager.get_compatibility_info = Mock(return_value={
...
def wsl1_environment():
    ...
    with patch('platform.system', return_value='Linux'), \
         patch('builtins.open', side_effect=lambda path, *args, **kwargs: 
               MagicMock(read=lambda: 'microsoft-standard-WSL1') if path == '/proc/version' 
               else MagicMock()):
        yield
...
def wsl2_environment():
    ...
    with patch('platform.system', return_value='Linux'), \
         patch('builtins.open', side_effect=lambda path, *args, **kwargs: 
               MagicMock(read=lambda: 'microsoft-standard-WSL2') if path == '/proc/version' 
               else MagicMock()):
        yield
...
def native_linux_environment():
    ...
    with patch('platform.system', return_value='Linux'), \
         patch('builtins.open', side_effect=lambda path, *args, **kwargs: 
               MagicMock(read=lambda: 'generic-linux-kernel') if path == '/proc/version' 
               else MagicMock()):
        yield