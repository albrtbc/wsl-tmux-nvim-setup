...
class TestRunner:
    ...
    def run_test_category(self, category: str, markers: List[str], 
                         timeout: int = 300, capture_output: bool = True) -> Dict:
        ...
        try:
            ...
            test_results = {
                ...
                'output': result.stdout if capture_output else f"Tests executed (duration: {duration:.2f}s)",
    ...
    def generate_report(self, output_file: Optional[Path] = None) -> Dict:
        ...
        report = {
            ...
            'summary': {
                ...
                'passed_categories': sum(1 for r in self.test_results.values() if r.get('passed', False)),
                'failed_categories': sum(1 for r in self.test_results.values() if not r.get('passed', True) and not r.get('skipped', False)),
                'skipped_categories': sum(1 for r in self.test_results.values() if r.get('skipped', False)),
...
Examples:
  ...
  python run_tests.py --report results.json  # Save detailed report
    ...
    parser.add_argument(
        '--check-requirements',
        action='store_true',
        help='Check test requirements and exit'
    )
    ...
    try:
        if args.category:
            ...
            result = runner.run_test_category(
                args.category,
                markers,
                timeouts.get(args.category, 300)
            )
